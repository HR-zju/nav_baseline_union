...
def main(args: argparse.Namespace):
    ...
    # ROS 2
    rclpy.init()
    node = Node("exploration_node")  # 创建节点
    ...
    # ROS 2
    image_curr_msg = node.create_subscription(Image, IMAGE_TOPIC, callback_obs, 1)
    waypoint_pub = node.create_publisher(Float32MultiArray, WAYPOINT_TOPIC, 1)
    sampled_actions_pub = node.create_publisher(Float32MultiArray, SAMPLED_ACTIONS_TOPIC, 1)
    goal_pub = node.create_publisher(Bool, "/topoplan/reached_goal", 1)
    ...
    try:
        ...
        while rclpy.ok():
            ...
            # RECOVERY MODE
            # print('chosen_waypoint = {}'.format(chosen_waypoint))
            # breakpoint()
            ...
            goal_pub.publish(reached_goal_msg)
            if reached_goal:
                print("Reached goal! Stopping...")
            rclpy.spin_once(node)  # 处理回调函数

            current_time = node.get_clock().now()
            time_diff = (current_time - prev_time).nanoseconds * 1e-9  # 转换为秒
            node.get_logger().info(f'Time difference: {time_diff} seconds')
